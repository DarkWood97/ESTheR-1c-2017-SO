...
typedef struct {
    ...
    int PC;
//  int paginas_Codigo;
//  t_metadata_program* cod;//Importar librerias
// etiquetas etiq;
	Stack SP;
// int EX;
} PCB;
...
//-----FUNCIONES KERNEL------------------------------------------
	kernel crearKernel(t_config *configuracion) {
		...
		kernelAuxiliar.puerto_Memoria = config_get_int_value(configuracion,
				"PUERTO_MEMORIA");
		kernelAuxiliar.ip_Memoria.numero = config_get_string_value(
				configuracion, "IP_MEMORIA");
		kernelAuxiliar.puerto_FS = config_get_int_value(configuracion,
				"PUERTO_FS");
		...
		kernelAuxiliar.grado_Multiprog = config_get_int_value(configuracion,
				"GRADO_MULTIPROG");
		kernelAuxiliar.sem_Ids = config_get_array_value(configuracion,
				"SEM_IDS");
		kernelAuxiliar.sem_Init = config_get_array_value(configuracion,
				"SEM_INIT");
	...
	paquete serializar(int tipoMensaje, void* bufferDeData) {
		...
		paqueteAEnviar.mensaje = malloc(tamPaquete); //by Gustavo Adolf
	...
	int recibirCantidadPaginas(void* dataRecibida, PCB pcb){
		int tamaño =(sizeof(pcb.SP)+sizeof(pcb.cod))//-10bytes;
	}
...
//-----------------------------------MAIN-----------------------------------------------
	...
	int main(int argc, char *argv[]) {
		...
		int socketParaMemoria, socketParaFileSystem, socketMaxCliente,
				socketMaxMaster, socketAChequear, socketAEnviarMensaje,
				socketQueAcepta, bytesRecibidos;
		int tamMsj = strlen(buff);
		int socketEscucha = ponerseAEscucharClientes(kernel.puerto_Prog, 0);
		socketParaMemoria = conectarAServer(kernel.ip_Memoria.numero,
				kernel.puerto_Memoria);
		socketParaFileSystem = conectarAServer(kernel.ip_FS.numero,
				kernel.puerto_FS);
